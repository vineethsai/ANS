...
class AgentRegistry:
    ...
    def register_agent(self, agent: Agent) -> None:
        ...
        try:
            ...
            try:
                ...
                certificate_serial = agent_cert.get_serial_number()
            ...
            if not skip_verification:
                ...
                try:
                    ...
                    # Use OCSP verification if available
                    use_ocsp = hasattr(self.ca, '_ocsp_client') and self.ca._ocsp_client is not None

                    if use_ocsp:
                        # Get OCSP response
                        try:
                            ocsp_response = self.ca.get_ocsp_response(certificate_serial)
                            if not self.ca.verify_certificate_chain(agent_cert, use_ocsp=True, ocsp_response=ocsp_response):
                                raise ValueError("Invalid agent certificate (failed OCSP verification)")
                        except Exception as e:
                            # Fall back to traditional verification if OCSP fails
    ...
    def verify_endpoint_record(self, record: Dict[str, Any], use_ocsp: bool = True) -> bool:
        """
        Verify an endpoint record's signature.

        Args:
            record: Endpoint record to verify
            use_ocsp: Whether to use OCSP for certificate verification
        ...
        try:
            ...
            # Verify registry certificate with OCSP if available
            if use_ocsp and hasattr(self.ca, '_ocsp_client') and self.ca._ocsp_client is not None:
                try:
                    # Get OCSP response for registry certificate
                    ocsp_response = self.ca.get_ocsp_response(registry_cert.get_serial_number())
                    if not self.ca.verify_certificate_chain(registry_cert, use_ocsp=True, ocsp_response=ocsp_response):
                        return False
                except Exception:
                    # Fall back to traditional verification if OCSP fails
