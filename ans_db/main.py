...
from ..crypto.ocsp import OCSPStatus
from ..db.models import init_db, AgentModel, RevokedCertificateModel, OCSPResponseModel
from ..schemas import (
    validate_request,
    validate_response,
...
<body>
    ...
    <h2>Security Endpoints</h2>

    <div class="endpoint">
        <div><span class="method">POST</span> <span class="path">/ocsp</span></div>
        <div class="description">Check certificate status using the Online Certificate Status Protocol (OCSP)</div>
    </div>
...
def get_registry(db: Session = Depends(get_db)) -> AgentRegistry:
    ...
    if registry is None:
        ...
        ca.set_ocsp_responder(registry._registry_cert)
    return registry
...
async def revoke_agent(
    ...
    try:
        ...
        # Get agent certificate serial number and revoke it
        agent_model = registry.db.query(AgentModel).filter_by(agent_id=agent_id).first()
        if agent_model and agent_model.certificate_serial:
            # Revoke the certificate through CA
            ca.revoke_certificate(agent_model.certificate_serial, reason)

            # Add to database revocation list
            revoked_cert = RevokedCertificateModel(
                serial_number=agent_model.certificate_serial,
                revocation_time=datetime.utcnow(),
                reason=reason or "Not specified"
            )
            registry.db.add(revoked_cert)
            registry.db.commit()
...
@app.post("/ocsp", tags=["security"], summary="OCSP certificate status check",
         description="Check certificate status using the Online Certificate Status Protocol")
async def ocsp_check(
    request: Request,
    ...
    Args:
        ocsp_request: Request containing the certificate serial number
    ...
    try:
        ...
        # Get revoked certificate data from database
        revoked_certs = registry.db.query(RevokedCertificateModel).all()
        revoked_serials = [
            (cert.serial_number, cert.revocation_time, cert.reason)
            for cert in revoked_certs
        ]

        # Generate OCSP response
        ocsp_response = registry.ca.get_ocsp_response(serial_number)
        ...
        log_security_event(
            "ocsp_error",
            {"serial_number": ocsp_request.serial_number, "error": str(e)},
            "public_api",
            request
        )
        return create_error_response("ocsp_response", str(e))
    except Exception as e:
        log_security_event(
            "unexpected_error",
            {"serial_number": ocsp_request.serial_number, "error": str(e)},
        ...
        return create_error_response("ocsp_response", f"Unexpected error: {e}")
